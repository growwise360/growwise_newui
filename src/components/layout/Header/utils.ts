import { MenuItem, SubmenuItem, VariantStyles } from './types';
import {
  VARIANT_STYLES,
...
export function isSubmenuItemPathActive(
  item: SubmenuItem,
  pathname: string | null,
  locale: string,
): boolean {
  const itemUrl = createLocaleUrl(item.href, locale);
  if (item.href !== '#' && pathname?.startsWith(itemUrl)) {
    return true;
  }
  if (item.hasSubmenu && item.submenuItems?.length) {
    return item.submenuItems.some((nested) => isSubmenuItemPathActive(nested, pathname, locale));
  }
  return false;
}

export function isDropdownItemPathActive(
  dropdownItem: {
    href: string;
    hasSubmenu?: boolean;
  ...
  if (dropdownItem.submenuItems?.length) {
    return dropdownItem.submenuItems.some((sub) => isSubmenuItemPathActive(sub, pathname, locale));
  }
  return false;
}
...
/** Ensure Academic → Math always has hub href + grade-band flyout (stale API payloads). */
export function normalizeAcademicMathNav(menuItems: MenuItem[]): MenuItem[] {
  return menuItems.map((item) => {
    if (item.key !== 'academic' || !item.dropdown?.items) {
      return item;
    }
