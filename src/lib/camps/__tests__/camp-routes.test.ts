...
describe('camp-routes', () => {
  it('reserves academic hub segments from dynamic slug routes', () => {
    ...
    expect(CAMPS_STATIC_PATH_SEGMENTS.has('summer-im1-get-ready-dublin-ca')).toBe(true);
    expect(CAMPS_STATIC_PATH_SEGMENTS.has('summer-im2-get-ready-dublin-ca')).toBe(true);
  ...
  it('excludes reserved segments from camp landing static params', () => {
    ...
    expect(slugs).not.toContain('summer-im1-get-ready-dublin-ca');
    expect(slugs).not.toContain('summer-im2-get-ready-dublin-ca');
