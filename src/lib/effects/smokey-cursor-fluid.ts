...
export function createSmokeyCursorFluid(
  canvasElement: HTMLCanvasElement,
  options: SmokeyCursorFluidOptions,
): () => void {
...
// Global variables
let canvas = canvasElement, gl, ext;
...
function initializeShaders() {
  ...
  const displayShaderSource = `
                ...
                void main () {
                    ...
                    float a = max(c.r, max(c.g, c.b)) * alphaScale;
...
function generateColor() {
  ...
  c.r *= options.colorIntensity;
  c.g *= options.colorIntensity;
  c.b *= options.colorIntensity;
  return c;
}
...
function updateFrame() {
  ...
  if (isMobile && frameTime - lastMobileFrameTime < 1000 / 30) {
    animationFrameId = requestAnimationFrame(updateFrame);
    return;
  }
  lastMobileFrameTime = frameTime;
...
function drawDisplay(target) {
  ...
  if (displayMaterial.uniforms.alphaScale) {
    gl.uniform1f(displayMaterial.uniforms.alphaScale, options.alphaScale);
  }
  blit(target, false);
}
...
function clickSplat(pointer) {
  ...
  color.r *= options.clickBoost;
  color.g *= options.clickBoost;
  color.b *= options.clickBoost;
...
function setupEventListeners() {
  ...
  const onVisibilityChange = () => {
    if (!document.hidden && running && animationFrameId === null) {
  ...
  addListener(window, "mousedown", onMouseDown, { passive: true });
  addListener(window, "mousemove", onMouseMove, { passive: true });
  addListener(window, "touchstart", onTouchStart, { passive: true });
  addListener(window, "touchmove", onTouchMove, { passive: true });
  addListener(window, "touchend", onTouchEnd, { passive: true });
