...
1. Never add raw blocking third-party `<script>` tags outside `next/script`. **GTM / GA / Meta Pixel:** load only via `ConsentAndAnalytics` → `AnalyticsAfterConsent` after the user accepts cookies (`GTMHead` uses `strategy="afterInteractive"`). Do not mount GTM in root `app/layout.tsx` without consent — that regresses Lighthouse Best Practices (third-party cookies). **Standalone GA** (when GTM is off): `lazyOnload` inside `AnalyticsAfterConsent`. **Meta Pixel:** interaction-based loading per `MetaPixel.tsx` unless explicitly changed.
